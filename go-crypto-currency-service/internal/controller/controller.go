...
func remove(s []currency.RsaKeyPair, i int) []currency.RsaKeyPair {
...
func (c *Controller) Mine(w http.ResponseWriter, r *http.Request) {
	...
	tr := currency.Transaction{Sender: "NULL", Recipient: c.blockchain.GetPublicKeyByNickname(keyNickname), Amount: 1, Signature: []byte("mining reward")}
	success := c.blockchain.RegisterTransaction(tr)
	if !success {
		w.WriteHeader(http.StatusInternalServerError)
		return
	}
	currentBlockData := currency.BlockData{Index: strconv.Itoa(lastBlock.Index - 1), Transactions: c.blockchain.PendingTransactions}
...
func (c *Controller) MineWithPublicKey(w http.ResponseWriter, r *http.Request) {
	...
	lastBlock := c.blockchain.GetLastBlock()
	previousBlockHash := lastBlock.Hash
	tr := currency.Transaction{Sender: "NULL", Recipient: key.Public, Amount: 1, Signature: []byte("mining reward")}
	success := c.blockchain.RegisterTransaction(tr)
	if !success {
		w.WriteHeader(http.StatusInternalServerError)
		return
...
func (c *Controller) RegisterNodeInExistingNetwork(w http.ResponseWriter, r *http.Request) {
	...
	nodeToRegister.Url = c.currentNodeURL
